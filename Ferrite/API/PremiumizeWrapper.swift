...
class Premiumize: OAuthDebridSource, ObservableObject {
    ...
    func getAuthUrl() throws -> URL {
        guard let urlComponents = URLComponents(string: baseAuthUrl) else {
            throw DebridError.InvalidUrl
        }
        var urlComponentsWithItems = urlComponents
        urlComponentsWithItems.queryItems = [
            URLQueryItem(name: "client_id", value: clientId),
            URLQueryItem(name: "response_type", value: "token"),
            URLQueryItem(name: "state", value: UUID().uuidString)
        ]

        if let url = urlComponentsWithItems.url {
            return url
        } else {
            throw DebridError.InvalidUrl
        }
