...
class Premiumize: OAuthDebridSource, ObservableObject {
    ...
    private func fetchDDL(magnet: Magnet) async throws -> DebridIA {
        ...
        guard let ddlUrl = URL(string: "\(baseApiUrl)/transfer/directdl") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: ddlUrl)
    ...
    private func createTransfer(magnet: Magnet) async throws {
        ...
        guard let createUrl = URL(string: "\(baseApiUrl)/transfer/create") else {
            throw DebridError.InvalidUrl
        }
    ...
    func getUserDownloads() async throws {
        guard let listAllUrl = URL(string: "\(baseApiUrl)/item/listall") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: listAllUrl)
    ...
    func deleteUserDownload(downloadId: String) async throws {
        guard let deleteUrl = URL(string: "\(baseApiUrl)/item/delete") else {
            throw DebridError.InvalidUrl
        }
