...
class RealDebrid: PollingDebridSource, ObservableObject {
    ...
    func getApiTokens(deviceCode: String) async throws {
        ...
        guard let tokenUrl = URL(string: "\(baseAuthUrl)/token") else {
            throw DebridError.InvalidUrl
        }
    ...
    func logout() async {
        ...
        if let token = FerriteKeychain.shared.get("RealDebrid.AccessToken") {
            if let logoutUrl = URL(string: "\(baseApiUrl)/disable_access_token") {
    ...
    func addMagnet(magnet: Magnet) async throws -> String {
        ...
        guard let addMagnetUrl = URL(string: "\(baseApiUrl)/torrents/addMagnet") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: addMagnetUrl)
        request.httpMethod = "POST"
        request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
    ...
    func selectFiles(debridID: String, fileIds: [Int]) async throws {
        guard let selectFilesUrl = URL(string: "\(baseApiUrl)/torrents/selectFiles/\(debridID)") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: selectFilesUrl)
        request.httpMethod = "POST"
        request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
    ...
    func torrentInfo(debridID: String) async throws -> TorrentInfoResponse {
        guard let infoUrl = URL(string: "\(baseApiUrl)/torrents/info/\(debridID)") else {
            throw DebridError.InvalidUrl
        }
    ...
    func unrestrictFile(_ restrictedFile: DebridIAFile) async throws -> String {
        guard let unrestrictUrl = URL(string: "\(baseApiUrl)/unrestrict/link") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: unrestrictUrl)
        request.httpMethod = "POST"
        request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
    ...
    func getUserMagnets() async throws {
        guard let torrentsUrl = URL(string: "\(baseApiUrl)/torrents") else {
            throw DebridError.InvalidUrl
        }
    ...
    func deleteUserMagnet(cloudMagnetId: String?) async throws {
        ...
        guard let deleteUrl = URL(string: "\(baseApiUrl)/torrents/delete/\(deleteId)") else {
            throw DebridError.InvalidUrl
        }
    ...
    func getUserDownloads() async throws {
        guard let downloadsUrl = URL(string: "\(baseApiUrl)/downloads") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: downloadsUrl)
    ...
    func deleteUserDownload(downloadId: String) async throws {
        guard let deleteUrl = URL(string: "\(baseApiUrl)/downloads/delete/\(downloadId)") else {
            throw DebridError.InvalidUrl
        }
    ...
    private func availableHost() async throws -> String? {
        guard let hostsUrl = URL(string: "\(baseApiUrl)/torrents/availableHosts") else {
            throw DebridError.InvalidUrl
        }
