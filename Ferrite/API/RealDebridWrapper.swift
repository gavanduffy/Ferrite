...
class RealDebrid: PollingDebridSource, ObservableObject {
    ...
    func getApiTokens(deviceCode: String) async throws {
        ...
        guard let apiUrl = URL(string: "\(baseAuthUrl)/token") else {
            throw DebridError.InvalidUrl
        }
    ...
    func logout() async {
        ...
        if let token = FerriteKeychain.shared.get("RealDebrid.AccessToken") {
            guard let apiUrl = URL(string: "\(baseApiUrl)/disable_access_token") else {
                return
            }
            var request = URLRequest(url: apiUrl)
    ...
    func addMagnet(magnet: Magnet) async throws -> String {
        ...
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents/addMagnet") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
        request.httpMethod = "POST"
    ...
    func selectFiles(debridID: String, fileIds: [Int]) async throws {
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents/selectFiles/\(debridID)") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
        request.httpMethod = "POST"
    ...
    func torrentInfo(debridID: String) async throws -> TorrentInfoResponse {
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents/info/\(debridID)") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
    ...
    func unrestrictFile(_ restrictedFile: DebridIAFile) async throws -> String {
        guard let apiUrl = URL(string: "\(baseApiUrl)/unrestrict/link") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
        request.httpMethod = "POST"
    ...
    func getUserMagnets() async throws {
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
    ...
    func deleteUserMagnet(cloudMagnetId: String?) async throws {
        ...
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents/delete/\(deleteId)") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
    ...
    func getUserDownloads() async throws {
        guard let apiUrl = URL(string: "\(baseApiUrl)/downloads") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
    ...
    func deleteUserDownload(downloadId: String) async throws {
        guard let apiUrl = URL(string: "\(baseApiUrl)/downloads/delete/\(downloadId)") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
        request.httpMethod = "DELETE"
    ...
    private func availableHost() async throws -> String? {
        guard let apiUrl = URL(string: "\(baseApiUrl)/torrents/availableHosts") else {
            throw DebridError.InvalidUrl
        }
        var request = URLRequest(url: apiUrl)
        let data = try await performRequest(request: &request, requestName: #function)
    ...
    private func buildTorrentUploadBody(fileUrl: URL) throws -> (Data, String) {
        ...
        guard
            ...
            let newlineData = "\r\n".data(using: .utf8),
            let footerData = "--\(boundary)--\r\n".data(using: .utf8)
        else {
            throw DebridError.FailedRequest(description: "Could not build the torrent upload body")
        }
