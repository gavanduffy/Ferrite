...
class RealDebrid: PollingDebridSource, ObservableObject {
    ...
    private func buildTorrentUploadBody(fileUrl: URL) throws -> (Data, String) {
        ...
        let line1 = "--\(boundary)\r\n".data(using: .utf8)
        let line2 = "Content-Disposition: form-data; name=\"file\"; filename=\"\(fileName)\"\r\n".data(using: .utf8)
        let line3 = "Content-Type: application/x-bittorrent\r\n\r\n".data(using: .utf8)
        let line4 = "\r\n".data(using: .utf8)
        let line5 = "--\(boundary)--\r\n".data(using: .utf8)

        guard let line1, let line2, let line3, let line4, let line5 else {
            throw DebridError.FailedRequest(description: "Could not encode multipart form data")
        }
