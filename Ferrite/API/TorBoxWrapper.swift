...
class TorBox: DebridSource, ObservableObject {
    ...
    private func buildTorrentUploadBody(fileUrl: URL) throws -> (Data, String) {
        ...
        guard let headerData = header.data(using: .utf8) else {
            throw DebridError.FailedRequest(description: "Failed to encode multipart header")
        }
        ...
        let footer = "\r\n--\(boundary)--\r\n"
        guard let footerData = footer.data(using: .utf8) else {
            throw DebridError.FailedRequest(description: "Failed to encode multipart footer")
        }
