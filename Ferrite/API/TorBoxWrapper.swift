...
class TorBox: DebridSource, ObservableObject {
    ...
    private func createTorrent(magnet: Magnet) async throws -> Int {
        guard let createUrl = URL(string: "\(baseApiUrl)/torrents/createtorrent") else {
            throw DebridError.InvalidUrl
        }
    ...
    private func myTorrentList() async throws -> [MyTorrentListResponse] {
        guard let myListUrl = URL(string: "\(baseApiUrl)/torrents/mylist") else {
            throw DebridError.InvalidUrl
        }

        var request = URLRequest(url: myListUrl)
    ...
    func deleteUserDownload(downloadId: String) async throws {
        guard let controlUrl = URL(string: "\(baseApiUrl)/webdl/controlwebdownload") else {
            throw DebridError.InvalidUrl
        }
    ...
    func deleteUserMagnet(cloudMagnetId: String?) async throws {
        ...
        guard let controlUrl = URL(string: "\(baseApiUrl)/torrents/controltorrent") else {
            throw DebridError.InvalidUrl
        }
    ...
    private func createWebDownload(link: String) async throws -> Int {
        guard let createUrl = URL(string: "\(baseApiUrl)/webdl/createwebdownload") else {
            throw DebridError.InvalidUrl
        }
    ...
    private func myWebDownloadList() async throws -> [WebDownloadListResponse] {
        guard let myListUrl = URL(string: "\(baseApiUrl)/webdl/mylist") else {
            throw DebridError.InvalidUrl
        }
