                    ...
                    if checkedForPlugins {
                        ...
                        case .sources:
                            if installedSources.isEmpty, pluginManager.availableSources.isEmpty {
                                EmptyInstructionView(title: "No Sources", message: "Add a plugin list in Settings", systemImage: "puzzlepiece.extension")
                            }
                        case .actions:
                            if installedActions.isEmpty, pluginManager.availableActions.isEmpty {
                                EmptyInstructionView(title: "No Actions", message: "Add a plugin list in Settings", systemImage: "bolt.horizontal")
                            }
                        }
                    } else {
                        ProgressView()
